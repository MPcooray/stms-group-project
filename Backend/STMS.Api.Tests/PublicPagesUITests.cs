...
namespace STMS.Api.Tests
{
    ...
    public class PublicPagesUITests : IDisposable
    {
        ...
        public PublicPagesUITests()
        {
            ...
            // Configure downloads (use a temp folder so tests can assert PDF files were created)
            ...
            // Allow overriding the default wait timeout via env var PUBLIC_WAIT_SECONDS for flaky CI/local runs
            int waitSeconds = 60;
            if (!int.TryParse(Environment.GetEnvironmentVariable("PUBLIC_WAIT_SECONDS"), out waitSeconds) || waitSeconds <= 0)
            {
                waitSeconds = 60;
            }
        ...
        // Polls the download folder for a new PDF file created within timeoutSeconds.
        private static bool WaitForPdfInDirectory(string downloadDir, int timeoutSeconds = 10)
        {
        ...
        private void NavigatePublicPagesForRecording(string tournamentId)
        {
            ...
            // Attempt export to PDF if an export button exists on the leaderboard page
            try
            {
                var exportBtn = _driver.FindElements(By.XPath("//button[contains(., 'Export') or contains(., 'Download') or contains(., 'PDF')]")).FirstOrDefault();
                if (exportBtn != null)
                {
                    // Clear any existing PDFs in download dir for a clean check
                    var downloadDir = Path.Combine(Path.GetTempPath(), "stms-public-downloads");
                    try { foreach (var f in Directory.GetFiles(downloadDir, "*.pdf")) File.Delete(f); } catch { }

                    // Diagnostic: list files before click
                    try
                    {
                        var before = Directory.GetFiles(downloadDir);
                        Console.WriteLine($"[DIAG] Downloads before export: {string.Join(", ", before.Select(Path.GetFileName))}");
                    }
                    catch { }
                    ...
                    // Poll longer for PDFs (30s) and show diagnostics after
                    var found = WaitForPdfInDirectory(downloadDir, timeoutSeconds: 30);
                    if (!found)
                    {
                        Console.WriteLine("[INFO] Export button clicked but no PDF detected in download directory.");
                        try
                        {
                            var after = Directory.GetFiles(downloadDir);
                            Console.WriteLine($"[DIAG] Downloads after export attempt: {string.Join(", ", after.Select(Path.GetFileName))}");
                        }
                        catch { }
                    }
                    else
                    {
                        Console.WriteLine("[INFO] PDF export detected in download directory.");
                        try
                        {
                            var after = Directory.GetFiles(downloadDir);
                            Console.WriteLine($"[DIAG] Downloads after export: {string.Join(", ", after.Select(Path.GetFileName))}");
                        }
                        catch { }
                    }
                }
                ...
                {
                    Info("No export button found on leaderboard page.");
                }
            }
            catch (Exception ex)
            {
                Info("Export check failed: " + ex.Message);
            }
            // TOURNAMENTS
        ...
        public void PublicPages_PublicFlowExistingTournament()
        {
            ...
            // ===== HOME =====
            _driver.Navigate().GoToUrl(FrontendBase + "/");
            _wait.Until(d => d.FindElement(By.CssSelector(".brand")));
